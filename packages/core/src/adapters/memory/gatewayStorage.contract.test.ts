...
export function describeGatewayStorageContract(
  ...
  describe(`${name} — GatewayStorage contract`, () => {
    it('starts with no identity and remembers the one it is told', () => {
      const store = create()
    ...
    it('round-trips a session and finds it by id', () => {
      const store = create()
      const session = {
    ...
    it('deletes every session for one device without touching the others', () => {
      const store = create()
    ...
    it('deletes only sessions that have actually expired', () => {
      const store = create()
    ...
    it('tracks a device epoch, defaulting to absent rather than zero', () => {
      const store = create()
    ...
    it('prunes expired nonces and reports the next expiry', () => {
      const store = create()
    ...
    it('replays a remembered ack instead of re-executing', () => {
      const store = create()
    ...
    it('assigns strictly increasing sequences across separate appends', () => {
      const store = create()
    ...
    it('reports no oldest sequence for an empty stream', () => {
      const store = create()
      expect(store.events.oldestSeq()).toBeNull()
      expect(store.events.latestSeq()).toBe(0)
    })

    it('keeps the newest rows when pruning, however old they are', () => {
    ...
    it('never renumbers surviving events after a prune', () => {
      const store = create()
    ...
    it('stores mailbox entries and identifies the oldest for eviction', () => {
      const store = create()
    ...
    it('reports no oldest mailbox entry when empty', () => {
      const store = create()
...
describeGatewayStorageContract('memory', createMemoryGatewayStorage)
